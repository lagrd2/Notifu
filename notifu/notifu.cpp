...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
	...
	ULONG *OrigMessageDuration=NULL;
	OrigMessageDuration = (ULONG *)calloc(sizeof(ULONG),1);
	if (OrigMessageDuration == NULL) return -1;
   ...
   if (gCommandLine.Continue())
   {
	  ...
	  if (IsWindowsVistaOrGreater()){
		  SystemParametersInfo(0x2016, 0, OrigMessageDuration, 0);//0x2016 = GetMessageDuration
	  }
      result = NotifyUser(params, &mqc, &unc);
	  if (IsWindowsVistaOrGreater()){
		  SystemParametersInfo(0x2017, 0, ULongToPtr(*OrigMessageDuration), 0);
	  }
      SerializeLeave();
